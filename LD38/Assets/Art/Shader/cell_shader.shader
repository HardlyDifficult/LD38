...
Shader "Dave/CellShader - Diffuse" {
	Properties{
...
#pragma fragment frag
		...
		float4 frag(vertexOutput i) : COLOR{
			...
			float3 lightFinal = UNITY_LIGHTMODEL_AMBIENT.xyz + specularReflection * _SpecColor + rimLighting;

			if (any(specularReflection > _SuperSpecCut)) {
				return float4(_SuperSpecColor.xyz, 1.0);
			}

			return float4(UNITY_LIGHTMODEL_AMBIENT.xyz + tex.xyz * (diffuseReflection > _ColorCut ? _Color.xyz : _SecColor) * _LightColor0.xyz, 1.0);
		}
			ENDCG
		}
