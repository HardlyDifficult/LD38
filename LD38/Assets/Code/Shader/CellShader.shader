...
Shader "Dave/CellShader" {
	...
	_BumpDepth("Bump Depth", Range(0.0, 10.0)) = 5
		...
		_SuperSpecColor("Spuer Spec Color", Color) = (1.0,1.0,1.0,1.0)
...
#pragma fragment frag
	...
	float4 frag(vertexOutput i) : COLOR{
		...
		float3 specularReflection = saturate(diffuseReflection) * _SpecColor.xyz * pow(saturate(dot(reflect(-lightDirection, normalDirection), viewDirection)), _Shininess);
		...
		float3 lightFinal = UNITY_LIGHTMODEL_AMBIENT.xyz + specularReflection + rimLighting;
		...
		return float4(tex.xyz * (diffuseReflection > _ColorCut ? _Color.xyz : _SecColor) * lightFinal * _LightColor0.xyz + (specularReflection < _SuperSpecCut ? float3(0, 0, 0) : _SuperSpecColor.xyz), 1.0);
	}
		ENDCG
	}
...
#pragma fragment frag
	...
	float4 frag(vertexOutput i) : COLOR{
		...
		float3 lightFinal = specularReflection + rimLighting;
		// float lFLength = length(lightFinal);
		// lightFinal = round((lightFinal / 3) * _PrimSpecMod) / _PrimSpecMod;
		lightFinal = round(float3(1.0, 1.0, 1.0) * length(lightFinal) * _PrimSpecMod) * _SecSpecMod;

		return float4((diffuseReflection > _ColorCut ? _Color.xyz : _SecColor) * lightFinal * _LightColor0.xyz + (specularReflection < _SuperSpecCut ? float3(0, 0, 0) : _SuperSpecColor.xyz), 1.0);
	}
		ENDCG
	}
	}
